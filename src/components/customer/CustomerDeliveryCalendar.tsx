...
export default function CustomerDeliveryCalendar({ userId }: CustomerDeliveryCalendarProps) {
    ...
    const handleSubmitRequest = async () => {
        if (!selectedOrder || !requestReason) {
            alert(language === 'tr' ? 'Lütfen değişiklik talebi için bir neden belirtin' : 'Please provide a reason for the change request');
            return;
        }
        ...
        try {
            ...
            alert(language === 'tr' ? 'Değişiklik talebiniz başarıyla iletildi!' : 'Change request submitted successfully!');
            ...
            alert(language === 'tr' ? 'Talep gönderilemedi' : 'Failed to submit request');
        } finally {
            setSubmitting(false);
        }
    };
    ...
    if (loading) {
        return <div className="text-center py-8 text-gray-500">{language === 'tr' ? 'Siparişleriniz yükleniyor...' : 'Loading your orders...'}</div>;
    }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-4 rounded-lg shadow">
                    <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
                        ...
                        {selectedDate.toLocaleDateString(language === 'tr' ? 'tr-TR' : 'en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
                    </h3>
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-2">
                                            <div>
                                                <p className="font-medium text-gray-900">{language === 'tr' ? 'Sipariş No' : 'Order #'} {order.id.slice(0, 8)}</p>
                                                <p className="text-sm text-gray-600">{order.total_amount} {order.currency}</p>
                                                <p className="text-xs text-gray-500 mt-1">
                                                    {language === 'tr' ? 'Sipariş Tarihi:' : 'Ordered:'} {new Date(order.created_at).toLocaleDateString(language === 'tr' ? 'tr-TR' : 'en-US')}
                                                </p>
                                            </div>
                                        ...
                                        >
                                            {language === 'tr' ? 'Tarih Değişikliği Talep Et' : 'Request Date Change'}
                                        </button>
                                    </div>
                                );
                            })}
            ...
            <div className="bg-white p-6 rounded-lg shadow">
                <h3 className="text-lg font-semibold mb-4">{language === 'tr' ? 'Siparişleriniz' : 'Your Orders'}</h3>
                                ...
                                >
                                    ...
                                    <div className="text-right">
                                        <p className="text-sm font-medium text-green-600">
                                            {new Date(order.delivery_date!).toLocaleDateString(language === 'tr' ? 'tr-TR' : 'en-US')}
                                        </p>
                                        <button
                                            onClick={() => {
                                                setSelectedOrder(order);
                                        ...
                                        >
                                            {language === 'tr' ? 'Tarih Değişikliği Talep Et' : 'Request Change'}
                                        </button>
                                    </div>
                                </div>
                            ))}
                                ...
                                >
                                    ...
                                    <div className="text-right">
                                        <p className="text-sm font-medium text-orange-600">
                                            {calculateEstimate(order.created_at).toLocaleDateString(language === 'tr' ? 'tr-TR' : 'en-US')}
                                        </p>
                                        <button
                                            onClick={() => {
                                                setSelectedOrder(order);
                ...
                {orders.length === 0 && (
                    <p className="text-gray-500 text-center py-8">{language === 'tr' ? 'Henüz hiç sipariş vermediniz' : "You haven't placed any orders yet"}</p>
                )}
            </div>
            ...
            {showRequestForm && selectedOrder && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
                        <h3 className="text-xl font-bold mb-4">{language === 'tr' ? 'Teslimat Tarihi Değişikliği Talebi' : 'Request Delivery Date Change'}</h3>

                        <div className="mb-4 p-4 bg-gray-50 rounded-lg">
                            <p className="text-sm text-gray-600 mb-2">{language === 'tr' ? 'Sipariş Detayları:' : 'Order Details:'}</p>
                            <p className="font-medium">{language === 'tr' ? 'Sipariş No' : 'Order #'} {selectedOrder.id.slice(0, 8)}</p>
                            <p className="text-sm text-gray-600">{selectedOrder.total_amount} {selectedOrder.currency}</p>
                            <p className="text-xs text-gray-500 mt-1">
                                {language === 'tr' ? 'Mevcut Teslimat:' : 'Current delivery:'} {selectedOrder.delivery_date
                                    ? new Date(selectedOrder.delivery_date).toLocaleDateString(language === 'tr' ? 'tr-TR' : 'en-US')
                                    : (language === 'tr' ? 'Belirtilmedi' : 'Not set')}
                            </p>
                        </div>

                        <div className="mb-4">
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                {language === 'tr' ? 'Tercih Edilen Yeni Tarih (İsteğe Bağlı)' : 'Preferred New Date (Optional)'}
                            </label>
                            <input
                                type="date"
                            ...
                            />
                            <p className="text-xs text-gray-500 mt-1">{language === 'tr' ? 'Belirli bir tarih yoksa boş bırakın' : "Leave empty if you don't have a specific date in mind"}</p>
                        </div>

                        <div className="mb-4">
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                {language === 'tr' ? 'Değişiklik Nedeni *' : 'Reason for Change *'}
                            </label>
                            <textarea
                                value={requestReason}
                                onChange={(e) => setRequestReason(e.target.value)}
                                placeholder={language === 'tr' ? 'Lütfen teslimat tarihini neden değiştirmek istediğinizi açıklayın...' : 'Please explain why you need to change the delivery date...'}
                                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                                rows={4}
                                required
                            />
                        ...
                        <div className="flex gap-3">
                            ...
                            >
                                {submitting ? t('submitting') : (language === 'tr' ? 'Talebi Gönder' : 'Submit Request')}
                            </button>
                            <button
                                onClick={() => {
                                    setShowRequestForm(false);
