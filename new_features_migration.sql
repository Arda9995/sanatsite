...
CREATE TABLE IF NOT EXISTS blacklist (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    type TEXT NOT NULL, -- 'country' or 'area'
    value TEXT NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- Admin only for all operations on pricing_metrics and blacklist
CREATE POLICY "Admins can do everything on pricing_metrics" ON pricing_metrics FOR ALL USING (auth.jwt() ->> 'email' IN (SELECT email FROM profile_metadata WHERE role = 'admin')); -- This assumes a way to identify admins, adjust based on existing patterns
CREATE POLICY "Admins can do everything on blacklist" ON blacklist FOR ALL USING (auth.jwt() ->> 'email' IN (SELECT email FROM profile_metadata WHERE role = 'admin'));

-- Public/Authenticated read access if needed
CREATE POLICY "Anyone can read pricing_metrics" ON pricing_metrics FOR SELECT USING (true);
